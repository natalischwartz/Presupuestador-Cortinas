...
export const CurtainTypeStep = ({ data, updateData }) => {
  ...
  return (
    <div className="space-y-6">
        ...
        {options.map((option) => {
              ...
              return (
                <Card
                  ...
                  onClick={() => option.available && updateData({ curtainType: option.id })}
                >
      ...
      {!options.find(opt => opt.available && opt.id === data.curtainType) && data.curtainType === 'roller' && (
            <div className="text-center p-4 bg-warning/10 rounded-lg border border-warning/20">
              <p className="text-warning-foreground">
                Las cortinas roller estarán disponibles próximamente. 
                Por ahora podés cotizar cortinas tradicionales.
              </p>
            </div>
          )}
    </div>
